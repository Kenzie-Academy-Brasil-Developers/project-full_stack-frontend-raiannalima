...
export const Modal_delete_anouncement = () => {
    const { removeAnouncement, currentAnouncement, closeModalDelete } = useContext(AnouncementContext);
    return (
        <div className="modal-wrapper w-full h-full bg-modalWrapper fixed top-0 left-0 flex items-center justify-center">
            <div className="modal w-[32.5rem] h-[18.9375rem] bg-whiteFixed rounded-lg py-4 px-6">
                <div className="modal-header flex justify-between mb-[2.5rem]">
                    ...
                    <button onClick={closeModalDelete} type="button" className="text-[25px] font-lexend text-grey4">
                        x
                    </button>
                </div>
                <div className="modal-main">
                    ...
                    <div className="flex justify-end">
                        <button onClick={closeModalDelete} type="button" className="w-[9.375rem] h-[3rem] bg-grey6 rounded font-semibold text-grey2 mr-[0.3125rem]">
                            Cancelar
                        </button>
                        <button onClick={() => removeAnouncement(currentAnouncement!.id)} type="button" className="w-[12.500rem] h-[3rem] bg-alert2 rounded font-semibold text-alert1 mr-[0.3125rem]">
                            Sim, excluir anúncio
                        </button>
                    </div>
                </div>
